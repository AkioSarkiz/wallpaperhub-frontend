...
export default async function WallpaperPage({ params }: WallpaperPageProps) {
    ...
    const wallpaper = await backendFetchOne<IWallpaperAPI>(`/api/v1/wallpapers/${id}`);
        ...
        <main className="container mx-auto px-4 py-8">
            <div className="grid gap-8 lg:grid-cols-2">
                ...
                <div className="flex flex-col">
                    <div className="mb-6">
                        <h1 className="mb-2 text-balance text-3xl font-bold text-foreground">{wallpaper.title}</h1>
                        <p className="text-muted-foreground">
                            by <span className="font-medium text-foreground">{wallpaper.user.name}</span>
                        </p>
                    </div>

                    <p className="mb-6 text-pretty leading-relaxed text-muted-foreground">{wallpaper.description}</p>

                    {/* Stats */}
                    <div className="mb-6 flex flex-wrap gap-4">
                        <div className="flex items-center gap-2 text-sm">
                            <Download className="h-4 w-4 text-muted-foreground" />
                            <span className="text-muted-foreground">{wallpaper.total_downloads} downloads</span>
                        </div>
                        <div className="flex items-center gap-2 text-sm">
                            <Eye className="h-4 w-4 text-muted-foreground" />
                            <span className="text-muted-foreground">{wallpaper.total_views} views</span>
                        </div>
                    </div>
                    ...
                    <div className="mb-8 rounded-lg border border-border bg-card p-4">
                        <h3 className="mb-3 text-sm font-medium text-card-foreground">Specifications</h3>
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                                <span className="text-muted-foreground">Resolution</span>
                                <span className="font-medium text-card-foreground">
                                    {wallpaper.wallpaper.metadata.height}x{wallpaper.wallpaper.metadata.width}
                                </span>
                            </div>
                            <div className="flex justify-between">
